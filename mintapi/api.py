...
def createReport(data, sheetName):
    ...
    sheetData = json.loads(value_range_body)
    for result in data:
        if result['accountSystemStatus'] == 'ACTIVE':
            sheetData["values"].append([result['fiLoginDisplayName'],result['currentBalance'],result['accountType'],result['lastUpdatedInDate']])

    request = service.spreadsheets().values().update(spreadsheetId=SPREADSHEET_ID, range=range_, valueInputOption=value_input_option, body=sheetData)
...
def getMintCode():
    ...
    for record in messages:
        messageList[record.date_created] = record.sid
    messageList = OrderedDict(sorted(messageList.items(),reverse=True))  
    latestMsg = list(messageList.values())[0]
    message = client.messages(latestMsg).fetch()

    mintCode = int(message.body.split(' ')[len(message.body.split(' '))-1])
    return mintCode
...
def main():
    ...
    # output the data
    createReport(data,'1uG3uWdnjVyDw-WZfFOlGRVtW4wuBvU7Df65ZlGnS0uc')
